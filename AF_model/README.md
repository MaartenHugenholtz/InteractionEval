...
* PyTorch == 1.8.0
### Dependencies:
1. Install [PyTorch 1.8.0](https://pytorch.org/get-started/previous-versions/) with the correct CUDA version. Installed on CPU using:
    ```
    install pytorch==1.8.0 torchvision==0.9.0 torchaudio==0.8.0 cpuonly -c pytorch
    ```
2. Install the dependencies:
    ```
    pip install -r requirements.txt
    ```
3. If there is an error with the pillow package when running, additionally run:
    ```
...
```
In order to run on CPU, use `--gpu -1` instead.
