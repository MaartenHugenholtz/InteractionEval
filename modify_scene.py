...
for i, b_comf in enumerate(mod_var):
    ...
    # if i == 0: # only for gt for the moment
    projected_trajs = project_trajectories(df)
    # add frame processing loop here 
    # plot_homotopy(projected_trajs)
    ...
    for frame in range(processor.init_frame+1, processor.num_fr - processor.future_frames):
        ...
        if data:
            ...
            # only store for ego agent 99
            if i == 0: # only for gt for the moment
                idx_ego = data['valid_id'].index(99)
                for idx_agent, agent in enumerate(data['valid_id']):
                    if agent != 99:
                        MLpred_interaction_angle = angle_diff_pred[0,idx_ego, idx_agent].item()
                        GT_interaction_angle = angle_diff_gt[0,idx_ego, idx_agent].item()
                        id_list.append(agent)
                        # MLpred_angle_list.append(MLpred_interaction_angle)
                        GT_angle_list.append(GT_interaction_angle)
                        frame_list.append(frame)

    plot_homotopy_angles = False

    if plot_homotopy_angles:
        df_angle = pd.DataFrame({
            't': frame_list,
            'agent_id': id_list,
            'ML_angle': MLpred_angle_list,
            'GT_angle': GT_angle_list,
        })
        
        # plot homotopy angles and predictions:
        fig = go.Figure()
        color_scale = px.colors.qualitative.Plotly + px.colors.qualitative.Plotly
        for j, agent_id in enumerate(df_angle['agent_id'].unique()):
            df_angle_agent = df_angle[df_angle['agent_id']==agent_id]
            fig.add_trace(go.Scatter(x = df_angle_agent['t'], y = df_angle_agent['GT_angle'],
                    mode = 'lines',
                    name = f'GT_angle_agent_{agent_id}',
                    showlegend=True,
                    line=dict(color=color_scale[-1] if agent_id == 99 else color_scale[int(agent_id)]),
                    ))
            fig.add_trace(go.Scatter(x = df_angle_agent['t'], y = df_angle_agent['ML_angle'],
                    mode = 'lines',
                    name = f'ML_angle_agent_{agent_id}',
                    showlegend=True,
                    line=dict(color=color_scale[-1] if agent_id == 99 else color_scale[int(agent_id)], dash = 'dash'),
                    )
                    )
            
        fig.update_layout(
            xaxis_title="Frame",
            yaxis_title="Homotopy angle difference to ego"
        )
        fig.show()
